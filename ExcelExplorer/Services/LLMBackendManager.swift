...
//  ExcelExplorer
//
//  Ported from AIStudio (Jordan Koch, 2026-02-19). Mirrors AIStudio's
//  balanced-dispatch LLM manager: OpenRouter frontier models, the optional Nova
//  Gateway, and a LoadBalancer over all local models — added on top of
//  ExcelExplorer's existing AIBackendManager (Ollama + OpenAI stay untouched).
//  Adapted for ExcelExplorer: ChatMessage → LLMChatMessage, AppSettings →
//  LLMAppSettings.
//
...
class LLMBackendManager: ObservableObject {
    ...
    init() {
        ...
        let settings = LLMAppSettings.shared
    ...
    func setActiveBackend(_ type: LLMBackendType) {
        ...
        LLMAppSettings.shared.activeLLMBackendType = type.rawValue
        determineActiveBackend()
    }
            ...
            if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
               let models = json["models"] as? [[String: Any]] {
                ...
                if !modelNames.isEmpty && !modelNames.contains(selectedOllamaModel) {
                    ...
                    LLMAppSettings.shared.selectedOllamaModel = modelNames[0]
                }
            }
            return .connected
        } catch {
    ...
    private func checkMLX() async -> BackendStatus {
        let pythonPath = LLMAppSettings.shared.pythonPath
        ...
        do {
            ...
            if !models.isEmpty {
                ...
                if !models.contains(selectedOpenRouterModel) {
                    ...
                    LLMAppSettings.shared.selectedOpenRouterModel = selectedOpenRouterModel
                }
            }
            return .connected
        } catch {
    ...
    private func buildOllamaMessages(prompt: String, systemPrompt: String?, messages: [LLMChatMessage]) -> [[String: String]] {
